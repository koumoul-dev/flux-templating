...
describe('Handlebars templater', function(){
  it('should apply a handlebars template to JSON data', function(callback) {
    app.document('application/json', 'text/plain', 'hello_world.txt', {
      who: 'World'
    }, function(err, result) {
  ...
  it('should get a 404 for missing template', function(callback) {
    app.document('application/json', 'text/plain', 'this_does_not_exist.txt', {
      who: 'World'
    }, function(err) {
      should.exist(err);
  ...
  it('should get a 400 for bad JSON input', function(callback) {
    app.document('application/json', 'text/plain', 'hello_world.txt', 'this is not JSON', function(err) {
      should.exist(err);
